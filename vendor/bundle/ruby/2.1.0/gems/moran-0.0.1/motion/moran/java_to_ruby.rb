...
class JavaToRuby
  ...
  def json_array_to_array(json_array)
    ...
    p = Profile.new('JARRAY TO HASH')
    v = Array.new.tap do |a|
      inx = 0
  ...
  def to_array(array)
    ...
    p = Profile.new('TO ARRAY')
    array.map { |value| convert_value(value) }
    p.end
  end

  def to_float(value)
    p = Profile.new('TO FLOAT')
    v = value.to_s.to_f
    p.end
    v
  end
  ...
  def to_boolean(value)
    p = Profile.new('TO BOOL')
    v = value.to_s.downcase == "true"
    p.end
    v
  end
...
class Profile
  ...
  def end
    puts @name+' ENDED: '+(XTime.now - @start).to_s
  end
end